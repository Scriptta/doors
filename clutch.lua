-- literally just clutch with a speed bypass and a broken wardrobe esp (trust me on this chat.. ctrl+f then type request or internal if u think it logs shit)
...
function esp(params: ESP)
    ...
    EspManager.rsConnection = RunService.RenderStepped:Connect(function()
        ...
        if Flags["ESPShowDistance"].Value then
            textLabel.Text = string.format("%s\n[%s]", EspManager.Text, math.floor(distanceFromCharacter(EspManager.Object:IsA("Model") and EspManager.Object:GetPivot().Position or EspManager.Object:IsA("BasePart") and EspManager.Object.Position)))
        else
            textLabel.Text = EspManager.Text
        end
...
function addWardrobeEsp(room)
    task.spawn(function()
        for i, v in room:WaitForChild'Assets':GetChildren() do
        ...
        Callback = function(value)
            if value then
                ...
                if collision then
                    ...
                    character:TranslateBy(Vector3.new(0, Flags["NoclipBypassOffset"].Value, 0))
                end
            else
                if collision then
...
local VisualsTab = Window:AddTab("Visuals") do
            ...
            Callback = function(value)
                if value then
                    ...
                    if character then
                        ...
                        Lighting.Ambient = Color3.fromRGB(138, 47, 75)
                    end
                end
            end
        })
    ...
    local Esp = VisualsTab:AddElementSection("ESP") do
        Esp:AddDropdown({
            ...
            Flag = "ESPWhat",
            Multi = true,
            Values = {"Door", "Wardrobe", "Entity", "Objective", "Item", "Dropped Item", "Gold", "Player"},
            Callback = function(value, oldValue)
                ...
                if value.Wardrobe ~= oldValue.Wardrobe then
                    if value.Wardrobe then
                        for i, v in workspace.CurrentRooms:GetChildren() do
                            warn'Yes'
                            addWardrobeEsp(v)
                        end
                    else
                        for i, v in pairs(espTable.Wardrobe) do
                            v:Destroy()
                        end
                    end
                end
        ...
        if alive then
            ...
            if humanoid then
                if Flags["Speed"].Value then
                    ...
                    if tick() - Timer >= .2 then -- main speed bypass
